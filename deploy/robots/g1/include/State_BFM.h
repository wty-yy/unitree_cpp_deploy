...
private:
    ...
    void initialize_q_target_filter(const YAML::Node& cfg, std::size_t dof);
    void reset_q_target_filter(const std::vector<float>& joint_pos);
    void apply_q_target_filter(std::vector<float>& q_target);
