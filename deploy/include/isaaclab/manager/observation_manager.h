...
protected:
    void _prapare_terms()
    {
        // Handle top-level special keys first
        if(this->cfg["use_gym_history"]) {
            use_gym_history = this->cfg["use_gym_history"].as<bool>();
        }

        // Check whether have multiple input groups
        // Find the first non-special key and check if it has "params" (flat single-group format)
        bool only_one_input = false;
        for(auto it = this->cfg.begin(); it != this->cfg.end(); ++it) {
            std::string key = it->first.as<std::string>();
            if(key == "use_gym_history" || key == "scale_first") continue;
            only_one_input = it->second["params"].IsDefined();
            break;
        }
    ...
    std::vector<ObservationTermCfg> _prepare_group_terms(const YAML::Node & group_cfg)
    {
        ...
        for(auto it = group_cfg.begin(); it != group_cfg.end(); ++it)
        {
            ...
            if(term_yaml_cfg["scale"] && !term_yaml_cfg["scale"].IsNull()) {
                term_cfg.scale = term_yaml_cfg["scale"].as<std::vector<float>>();
            }
            if(term_yaml_cfg["clip"] && !term_yaml_cfg["clip"].IsNull()) {
